...
export function buildIncrementalExplainPrompt(
  ...
  return [
    ...
    "",
    "CRITICAL RULES:",
    "- Return a PARTIAL JSON containing ONLY the fields that changed. Omit unchanged fields entirely.",
    "- ALWAYS include \"summary\": a clean one-line description of what the function does NOW (NOT what was changed).",
    "- The summary must describe the function's current purpose and behavior, as if explaining it fresh.",
    "- BAD summary: \"Changed variable i to j and added debug statement\" — this describes the edit, not the function.",
    "- GOOD summary: \"Fetches job details by ID, validates permissions, and returns a formatted response\" — this describes what the function does.",
    "- For \"stepByStep\": if any step changed, return the FULL updated array with all steps. Use `L<number>:` prefixes for line references.",
    "- Omit fields like inputs, outputs, dependencies if they did not change.",
    "",
