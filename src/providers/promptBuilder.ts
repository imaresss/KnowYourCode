...
export function buildIncrementalExplainPrompt(
  ...
  return [
    ...
    changeDescription,
    "",
    "CRITICAL: Return a PARTIAL JSON with ONLY the fields that need updating.",
    "- OMIT any field whose value is unchanged (do NOT echo back unchanged fields)",
    "- If only stepByStep changed, return ONLY {\"stepByStep\":[...], \"summary\":\"...\"}",
    "- If the change is trivial (unused variable, formatting), return {\"summary\":\"updated summary mentioning the change\"}",
    "- Always include \"summary\" so it reflects the current state",
    "- For stepByStep: if included, return the FULL updated array (all steps, with the affected ones modified)",
    "- Use `L<number>:` prefixes for line references based on new line numbers",
    "",
